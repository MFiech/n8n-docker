...
class N8NWorkflowServer {
  ...
  private activeTransports: Set<SSEServerTransport> = new Set();
  ...
  private setupNotificationHandlers() {
    // Handle MCP notifications like 'notifications/initialized'
    ...
    this.server["_notificationHandlers"].set('notifications/progress', async (notification: any) => {
      this.log('debug', 'Received progress notification', notification.params);
    });
  }
  ...
  private async startHttpServer(port: number): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // GET /sse endpoint for SSE transport
        app.get('/sse', (req: Request, res: Response) => {
          this.log('info', 'Setting up SSE connection');
          ...
          try {
            ...
            this.server.connect(transport).catch(error => {
              this.log('error', 'Error connecting SSE transport', error);
              this.activeTransports.delete(transport);
            });
          } catch (error) {
            this.log('error', 'Error setting up SSE connection', error);
            res.status(500).json({ error: 'Failed to establish SSE connection' });
          }
        });

        // POST /sse endpoint for Cursor StreamableHTTP transport
        app.post('/sse', (req: Request, res: Response) => {
          try {
            this.log('info', 'Cursor StreamableHTTP request to /sse', req.body);
            
            // Handle this exactly like /mcp endpoint
            this.handleJsonRpcMessage(req.body).then(result => {
              this.log('debug', 'Sending StreamableHTTP response', result);
              res.json(result);
            }).catch((error: Error) => {
              this.log('error', 'Error handling StreamableHTTP request', error);
              res.status(500).json({
                jsonrpc: '2.0',
                error: {
                  code: -32603,
            ...
            this.log('error', 'Error processing StreamableHTTP request', error);
            res.status(500).json({
              jsonrpc: '2.0',
              error: {
                code: -32603,
        ...
        // POST /messages endpoint for MCP message handling
        app.post('/messages', (req: Request, res: Response) => {
          try {
            this.log('info', 'MCP message request to /messages', req.body);
            
            this.handleJsonRpcMessage(req.body).then(result => {
              this.log('debug', 'Sending MCP message response', result);
              res.json(result);
            }).catch((error: Error) => {
              this.log('error', 'Error handling MCP message request', error);
              res.status(500).json({
                jsonrpc: '2.0',
                error: {
                  code: -32603,
            ...
            this.log('error', 'Error processing MCP message request', error);
            res.status(500).json({
              jsonrpc: '2.0',
              error: {
                code: -32603,
  ...
  private async handleJsonRpcMessage(request: any): Promise<any> {
    ...
    if (id === undefined || id === null) {
      this.log('info', `Handling notification: ${method}`);
      const notificationHandlers = this.server['_notificationHandlers'];
      
      if (notificationHandlers && notificationHandlers.has(method)) {
        try {
          await notificationHandlers.get(method)(request);
          // No response for notifications
          return null;
        } catch (error) {
          this.log('error', `Notification handler error: ${error instanceof Error ? error.message : String(error)}`);
          // Still no response for notifications, even on error
          return null;
        }
      } else {
        this.log('warn', `No notification handler for: ${method}`);
        return null;
      }
    }
